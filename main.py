    ...
    def update_canvas(self, *args):
        ...
        with self.canvas:
            ...
            if self.direction == "From North":
                raw_p1 = (raw_nw[0] + (raw_sw[0] - raw_nw[0]) * self.cut_frac[0], raw_nw[1] + (raw_sw[1] - raw_nw[1]) * self.cut_frac[0])
                raw_p2 = (raw_ne[0] + (raw_se[0] - raw_ne[0]) * self.cut_frac[1], raw_ne[1] + (raw_se[1] - raw_ne[1]) * self.cut_frac[1])
            elif self.direction == "From South":
                raw_p1 = (raw_sw[0] + (raw_nw[0] - raw_sw[0]) * self.cut_frac[0], raw_sw[1] + (raw_nw[1] - raw_sw[1]) * self.cut_frac[0])
                raw_p2 = (raw_se[0] + (raw_ne[0] - raw_se[0]) * self.cut_frac[1], raw_se[1] + (raw_ne[1] - raw_se[1]) * self.cut_frac[1])
            elif self.direction == "From East":
                raw_p1 = (raw_ne[0] + (raw_nw[0] - raw_ne[0]) * self.cut_frac[0], raw_ne[1] + (raw_nw[1] - raw_ne[1]) * self.cut_frac[0])
                raw_p2 = (raw_se[0] + (raw_sw[0] - raw_se[0]) * self.cut_frac[1], raw_se[1] + (raw_sw[1] - raw_se[1]) * self.cut_frac[1])
            elif self.direction == "From West":
                raw_p1 = (raw_nw[0] + (raw_ne[0] - raw_nw[0]) * self.cut_frac[0], raw_nw[1] + (raw_ne[1] - raw_nw[1]) * self.cut_frac[0])
                raw_p2 = (raw_sw[0] + (raw_se[0] - raw_sw[0]) * self.cut_frac[1], raw_sw[1] + (raw_se[1] - raw_se[1]) * self.cut_frac[1])
...
class AdvancedLandCalculatorApp(App):
    ...
    def compute_exact_c2(self, c1, target_sqft):
        ...
        for _ in range(40):
            mid = (low + high) / 2
            f1 = c1 / w if direction in ["From North", "From South"] else c1 / n
            f2 = mid / e if direction in ["From North", "From South"] else mid / s
    ...
    def on_cut1_change(self, instance, value):
        ...
        try:
            ...
            # Determine maximum allowable boundary for Side 1 based on direction
            if direction in ["From North", "From South"]:
                max_allowed = w
                side_name = "West Side Boundary"
            else:
                max_allowed = n
                side_name = "North Side Boundary"

            # Check if input exceeds main land boundary or is negative
            ...
            # Check if resulting Cut Side 2 exceeds its boundary as well
            max_c2_allowed = e if direction in ["From North", "From South"] else s
            if c2 > max_c2_allowed:
                self.updating_internally = True
                self.cut2_in.text = f"{c2:.2f} (Exceeds)"
                self.updating_internally = False

                c2_side_name = "East Side Boundary" if direction in ["From North", "From South"] else "South Side Boundary"
                err_msg = f"[color=ff4d4d][b]Error: Calculated Cut Side 2 exceeds boundary![/b]\n"
                err_msg += f"• Required Cut Side 2: [color=ffaa00]{c2:.2f} ft[/color]\n"
                err_msg += f"• Referenced {c2_side_name}: [color=00ff66]{max_c2_allowed:.2f} ft[/color]\n"
                err_msg += f"Reduce Cut Side 1 value to fit within the land limits.[/color]"
